...
def query_top_3_priority():
    query = """
    SELECT
        task,
        MAX((julianday(end_time) - julianday(start_time)) * 24) as total_hours,
        CAST((julianday(end_time) - julianday(start_time)) * 24 * 60 AS INT) % 60 as total_minutes
    FROM todo
